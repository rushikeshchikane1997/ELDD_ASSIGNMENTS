...
devices = (struct pchar_device*)kmalloc(devcnt * sizeof(struct pchar_device), GFP_KERNEL);
    ...
    for(i=0; i<devcnt; i++) {
        ...
        cdev_init(&devices[i].cdev, &pchar_fops); //hence we are making multi devices therefore it is taking addr of &devices[i].cdev
        ret = cdev_add(&devices[i].cdev, devno, 1);  // hence we are making multi devices therefore it is taking addr of &devices[i].cdev  
        if(ret != 0) {

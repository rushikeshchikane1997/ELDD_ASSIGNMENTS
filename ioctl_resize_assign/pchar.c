...
static long pchar_ioctl(struct file *pfile, unsigned int cmd, unsigned long param) {
	...
	switch (cmd)
	{
		...
		case FIFO_RESIZE:
			...
			// Declare a variable to store the new size
			long new_size;

			// Copy the new size from user space
			if (copy_from_user(&new_size, (long __user *)param, sizeof(long))) {
				printk(KERN_ERR "%s: pchar_ioctl() copy_from_user() failed.\n", THIS_MODULE->name);
				return -EFAULT;
			}

			// Get the current FIFO length
			long fifo_len = kfifo_len(&buf);

			// Allocate a temporary buffer of current FIFO length
			unsigned char *temp_buf = kmalloc(fifo_len, GFP_KERNEL);
			if (!temp_buf) {
			...
			int ret = kfifo_out(&buf, temp_buf, fifo_len);
...
MODULE_LICENSE("GPL");
MODULE_AUTHOR("rushikesh chikane");
